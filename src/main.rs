...
fn main() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        // let filename: PathBuf = [r"assets\", &date.to_string()[..], "_fullscreengrab.png"]
        //     .iter()
        //     .collect();
        ...
        std::thread::sleep(Duration::from_secs(3));
        let _ = cv_utils::GameWindow::crop_rh_weapon(PathBuf::from(filename)).unwrap();
        std::thread::sleep(Duration::from_secs(3));
    }

    // let comp = PathBuf::from("fullscreengrab.png");
    // let res1 = cv_utils::run_match_template(comp).unwrap();
    // res1.save("res1.png").unwrap();
    // open::that("res1.png");
    Ok(())
}
// +=====+======+ MAIN +=====+======+
...
//     std::fs::copy("starting_souls.png", "screenshots/fullscreengrab.png")?;
//     // crop it
